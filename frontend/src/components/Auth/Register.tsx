...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitted(true);
    await dispatch(register({ email, password, full_name: fullName }));
  };
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ marginBottom: 16 }}>
          ...
          <input
            ...
            type="text"
            value={fullName}
            onChange={(e) => { setFullName(e.target.value); if (error) dispatch(clearError()); }}
            required
        ...
        <button type="submit" disabled={isLoading || !email || !password || !fullName} style={{ width: '100%', padding: 12, background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: 6, fontWeight: 600 }}>
